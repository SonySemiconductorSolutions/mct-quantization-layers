    ...
    def _weights_lut_quantizer_test(self, inferable_quantizer, num_bits, threshold, lut_values,
        ...
        if per_channel:
            for i in range(len(threshold)):
                ...
                self.assertTrue(np.allclose(centers / (2 ** (lut_values_bitwidth - 1)) * threshold[i], channel_slice_i),
            ...
            self.assertTrue(np.allclose(centers / (2 ** (lut_values_bitwidth - 1)) * threshold[0], quantized_tensor),
                            "Quantized tensor values weren't assigned correctly")
